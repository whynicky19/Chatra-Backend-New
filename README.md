...
- **Тематику фона выбирает модель по названию предмета.** В промпт уезжает
  название класса («Веб-дизайн» → wireframes, сетки, vector paths), а
  `ICONS[icon]["motif"]` задаёт лишь планку конкретности и удерживает
  коллекцию в одном языке. Название пользовательское, поэтому проходит через
...
- **Элементы фона — часть фона.** Тонкие линии, сетки, волны, частицы,
  полупрозрачные и растворяющиеся в градиенте. Прямо запрещены коллаж из
  отдельных картинок, ряд иконок, плотные непрозрачные формы и перегруженный
  кадр.
