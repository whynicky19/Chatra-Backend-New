...
Клиент (веб/приложение)
  ...
  └─ POST /classes/{id}/cover/generate   {color, icon}
       ...
       ├─ OpenAI Images API:      абстрактный фон
       ├─ services/cover_art.py:  предметная иконка поверх (Pillow)
...
- **Иконку рисует код, а не модель.** Генеративные модели нестабильно
  воспроизводят символы (Σ, спираль ДНК, колба) — выходит псевдотекст и
  зеркальные буквы. Модель отвечает только за абстрактный фон, поверх
  которого `services/cover_art.py` рисует чёткий глиф примитивами Pillow.
  Композитинг на бэкенде, а не оверлеем в UI: обложка показывается больше
  чем в десяти местах веба и приложения, и все они уже умеют показывать одну
  картинку по `cover_image`/`cover_thumbnail` — править их не пришлось.
- **Единый стиль коллекции.** Промпт целиком строит бэкенд
  (`cover_art.build_prompt`); собственного промпта у преподавателя нет.
  Меняются только цвет и мотив, каркас стиля один. «Перегенерировать» меняет
  раскладку композиции (Images API не принимает seed), сохраняя цвет и иконку.
...
- **Обложки не может не быть.** Любой сбой (нет `OPENAI_API_KEY`, ошибка или
  ...
  локальному фолбэку в том же визуальном языке — градиент выбранного цвета,
  мягкая геометрия, тот же глиф. Ошибку наверх отдаёт только недоступное
  хранилище.
- **Стоимость.** `gpt-image-1-mini`, `quality=low`, `1536x1024` — **~$0.006
