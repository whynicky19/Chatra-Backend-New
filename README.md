...
Преподаватель больше не загружает фотографию: он выбирает **цвет** и
**предметную иконку**, а обложку рисует бэкенд. Один и тот же эндпоинт
обслуживает и веб, и мобильное приложение, поэтому обложка у предмета ровно
одна и она одинаковая везде.
...
Клиент (веб/приложение)
  ...
  └─ POST /classes/{id}/cover/generate   {color, icon}
       ├─ services/cover_art.py:  единый Chatra-промпт
       ├─ OpenAI Images API:      ТОЛЬКО абстрактный фон
       ├─ services/image_processing.py: WebP ≤1600px + миниатюра ≤480px
       └─ R2 materials/covers/ → cover_image + cover_thumbnail
  └─ GET  /classes/cover/options  — палитра и набор иконок для пикеров

Показ обложки (иконка живёт здесь, а не в картинке):
...
- **Иконку рисует UI, а не модель.** Генеративные модели нестабильно
  воспроизводят символы (Σ, спираль ДНК, колба) — выходит псевдотекст и
  зеркальные буквы. Модель отвечает только за фон; предметная иконка — это
  обычный SVG на клиенте, одного размера и толщины линии во всех предметах.
  Её можно перекрасить или заменить без перегенерации всех обложек.
- **Обложка — редакторская графическая иллюстрация, а не дашборд.** Одна
  цельная абстрактная композиция из немногих крупных форм: перетекающие
  кривые, дуги, круги, органика, наложение и прозрачность. Сюда уже заводили
  сетки, точечные матрицы и «чистые полосы под иконку» — ровно они и делали
  картинку похожей на SaaS-фон, поэтому в промпте они теперь прямо запрещены
  (`do not arrange the elements into a grid`, `no repetitive dot grids`,
  `nothing that looks like a SaaS dashboard`).
- **Координат модели не задаём.** Ни «левая треть», ни «чистая середина» —
  только художественное направление (`balanced asymmetrical composition`,
  `strong focal area`, `generous breathing room`). Композицию модель строит
  сама, и именно это вытащило результат из дашбордной эстетики.
- **Пустой зоны под иконку нет.** Иконка ложится поверх готовой композиции, а
  читаемость обеспечивает сам слой: белый глиф с мягкой тенью
  (`ICON_ON_ARTWORK`). Вариант «иконка в тон» здесь был и на нынешних
  насыщенных цветах давал контраст 1.5-2.0 — глиф просто пропадал.
- **Цвет — основа композиции.** В `PALETTE[color]["prompt"]` описана не одна
  краска, а СЕМЬЯ оттенков одного тона («luminous cornflower, sky blue and
  soft cobalt, with lighter azure tints…»). Слова `deep`/`dark` оттуда убраны
  намеренно: с ними модель уводила обложки в почти чёрное (замеренная средняя
  яркость 65-100 из 255 против 96-142 сейчас).
- **Мотив предмета — абстрактный визуальный язык**, а не список фигур:
  «abstract physics visual language: flowing waves, orbital curves, particles,
  energy fields». Перечисление фигур («сетка, точки, прямоугольники») как раз
  и порождало инфографику.
- **Единый стиль коллекции.** Промпт целиком строит бэкенд
  (`cover_art.build_prompt`); собственного промпта у преподавателя нет.
  «Перегенерировать» меняет только художественное направление композиции —
  одно из шести (`_COMPOSITIONS`), — сохраняя цвет, мотив и стиль.
- **Названия предмета в картинке нет.** Промпт явно запрещает любой текст —
  название, код курса и метаданные рисует UI поверх/рядом.
- **Обложки не может не быть.** Любой сбой (нет `OPENAI_API_KEY`, ошибка или
  ...
  локальному фолбэку в том же визуальном языке — насыщенный цвет одной семьи,
  несколько крупных перетекающих форм с наложением. Ошибку наверх отдаёт
  только недоступное хранилище.
- **Стоимость.** `gpt-image-1-mini`, `quality=medium`, `1024x1024` — **~$0.011
  за генерацию**. Расход пишется в `ai_usage_logs` с `endpoint="cover_image"`:
