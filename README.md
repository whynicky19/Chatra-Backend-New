...
- **Иконку рисует UI, а не модель.** Генеративные модели нестабильно
  ...
  Её можно перекрасить или заменить без перегенерации всех обложек. Чтобы
  иконке всегда было куда лечь, и промпт, и локальный фолбэк обязаны
  оставлять **центр кадра чистым** (`_CLEAR_CENTRE_RADIUS`); в фолбэке это
  обеспечивается маской, а не геометрией каждого элемента.
- **Обложка — светлая карточка курса, а не «AI-картинка про математику».**
  Пастельный фон выбранного цвета с мягким градиентом, деликатная текстура
  из оттенков того же тона (точечные сетки, штриховка, волновые ленты, дуги)
  по углам и краям, чистый центр. Никаких тёмных и насыщенных заливок,
  крупных центральных символов и случайных объектов.
- **Иконка — в цвет обложки, а не белая.** В палитре у каждого цвета три
  значения: `hex` (акцент бренда, свотч в пикере), `base` (светлая пастельная
  заливка фона) и `ink` (насыщенный тон иконки). Контраст `ink` на `base`
  держится тестом в узком коридоре, иначе один предмет выглядел бы бледнее
  другого и коллекция разваливалась бы.
- **Единый стиль коллекции.** Промпт целиком строит бэкенд
  (`cover_art.build_prompt`); собственного промпта у преподавателя нет.
  Меняются только цвет и декоративный мотив предмета (`ICONS[...]["motif"]` —
  «сетка + диагонали + дуги», «орбиты + частицы + волна» и т.п.), каркас
  стиля один. «Перегенерировать» меняет раскладку композиции (Images API не
  принимает seed), сохраняя цвет и иконку.
...
- **Обложки не может не быть.** Любой сбой (нет `OPENAI_API_KEY`, ошибка или
  ...
  локальному фолбэку в том же визуальном языке — заливка выбранного цвета,
  мягкие формы по краям, свободный центр. Ошибку наверх отдаёт только
