...
- **Элементы фона — часть фона, но видимая.** Тонкие линии, сетки, волны,
  частицы, полупрозрачные и растворяющиеся в градиенте — и при этом читаемые
  на маленькой карточке (`clearly visible and easy to read`, `not a barely
  perceptible texture`). Первая версия просила «very low contrast» и элементы
  пропадали вовсе. Прямо запрещены коллаж из отдельных картинок, ряд иконок,
  плотные непрозрачные формы и перегруженный кадр.
...
- **Экспозицию гарантирует арифметика, а не промпт.**
  ...
  Работает мультипликативно (тон и насыщенность не уезжают): сначала три
  прохода широкой радиальной маской гасят «прожектор», потом общая яркость
  подтягивается в коридор, ослабление ограничено двумя разами. Обложка, уже
