...
def _mk_thread(client, user):
    resp = client.post("/ai/threads", headers=auth_headers(user))
...
def test_list_threads_ordering(client, db_session, monkeypatch):
    ...
    client.patch(f"/ai/threads/{a['id']}", json={"pinned": True},
                 headers=auth_headers(user))

    listed = client.get("/ai/threads", headers=auth_headers(user)).json()
...
def test_patch_renames_and_pins(client, db_session):
    ...
    renamed = client.patch(f"/ai/threads/{t['id']}", json={"title": "Матан"},
                           headers=auth_headers(user)).json()
    assert renamed["title"] == "Матан"
    assert renamed["pinned"] is False

    pinned = client.patch(f"/ai/threads/{t['id']}", json={"pinned": True},
                          headers=auth_headers(user)).json()
...
def test_delete_thread_removes_messages(client, db_session, monkeypatch):
    ...
    resp = client.delete(f"/ai/threads/{t['id']}", headers=auth_headers(user))
...
def test_cannot_access_other_users_thread(client, db_session):
    ...
    assert client.get("/ai/history", params={"thread_id": t["id"]},
                      headers=auth_headers(b)).status_code == 404
    assert client.patch(f"/ai/threads/{t['id']}", json={"title": "hijack"},
                        headers=auth_headers(b)).status_code == 404
    assert client.delete(f"/ai/threads/{t['id']}",
                         headers=auth_headers(b)).status_code == 404
    # Тред a не пострадал.
    assert client.get("/ai/threads", headers=auth_headers(a)).json()[0]["id"] == t["id"]
...
def test_main_chat_roundtrip_persists_and_bumps_updated_at(client, db_session, monkeypatch):
    ...
    hist = client.get("/ai/history", params={"thread_id": t["id"]},
                      headers=auth_headers(user)).json()
    ...
    after = client.get("/ai/threads", headers=auth_headers(user)).json()[0]
...
def test_title_only_generated_on_first_exchange(client, db_session, monkeypatch):
    ...
    client.patch(f"/ai/threads/{t['id']}", json={"title": "Мой чат"},
                 headers=auth_headers(user))
    resp = client.post("/ai/chat",
...
def test_class_tutor_path_unaffected(client, db_session, monkeypatch):
    ...
    hist = client.get("/ai/history", params={"class_id": 42},
                      headers=auth_headers(user)).json()
