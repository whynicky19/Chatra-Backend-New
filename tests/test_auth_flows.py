...
def _register_and_verify(client, email, password="password123", org_type="university"):
    ...
    code = client.post("/auth/resend-verification",
                       json={"email": email, "org_type": org_type}).json()["dev_code"]
    r = client.post("/auth/verify-email", json={"email": email, "org_type": org_type, "code": code})
...
def test_login_blocked_until_email_verified(client):
    ...
    code = client.post("/auth/resend-verification", json={"email": "nv@x.com"}).json()["dev_code"]
    assert client.post("/auth/verify-email", json={"email": "nv@x.com", "code": code}).status_code == 200
...
def test_verify_email_wrong_code_rejected(client):
    ...
    bad = client.post("/auth/verify-email", json={"email": "wc@x.com", "code": "000000"})
...
def test_verify_returns_tokens_and_carries_token_version(client):
    ...
    assert client.get("/auth/me", headers=_bearer(tokens["access_token"])).status_code == 200
...
def test_forgot_and_reset_password(client):
    ...
    dev = client.post("/auth/forgot-password", json={"email": "fp@x.com"}).json()
    ...
    assert client.get("/auth/me", headers=_bearer(old["access_token"])).status_code == 401
...
def test_change_password_revokes_old_token_and_issues_new(client):
    ...
    assert client.get("/auth/me", headers=_bearer(new_access)).status_code == 200
...
def test_logout_revokes_refresh_token(client):
    ...
    assert client.post("/auth/logout", headers=_bearer(access)).status_code == 204
...
def test_delete_account_requires_password_and_removes_user(client):
    ...
    bad = client.request("DELETE", "/auth/me", json={"password": "wrong"}, headers=_bearer(access))
    assert bad.status_code == 400

    ok = client.request("DELETE", "/auth/me", json={"password": "password123"}, headers=_bearer(access))
