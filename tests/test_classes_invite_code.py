...
def test_join_by_code_happy_path(client, db_session):
    ...
    members = client.get(f"/classes/{cls.id}/members", headers=auth_headers(teacher)).json()
...
def test_invite_code_hidden_from_student_visible_to_owner_and_admin(client, db_session):
    ...
    r_student = client.get(f"/classes/{cls.id}", headers=auth_headers(student))
    assert r_student.json()["invite_code"] is None

    r_owner = client.get(f"/classes/{cls.id}", headers=auth_headers(teacher))
    assert r_owner.json()["invite_code"] == cls.invite_code

    r_admin = client.get(f"/classes/{cls.id}", headers=auth_headers(admin))
    assert r_admin.json()["invite_code"] == cls.invite_code

    # A teacher who doesn't own the class shouldn't see its code either.
    r_other = client.get(f"/classes/{cls.id}", headers=auth_headers(other_teacher))
    assert r_other.json()["invite_code"] is None

    # /classes/all never exposes the code, even to the owner.
    r_all = client.get("/classes/all", headers=auth_headers(teacher))
...
def test_regenerate_code_changes_code_and_invalidates_old_one(client, db_session):
    ...
    resp = client.post(f"/classes/{cls.id}/regenerate-code", headers=auth_headers(teacher))
    assert resp.status_code == 200
    new_code = resp.json()["invite_code"]
    assert new_code != old_code

    old_resp = client.post("/classes/join-by-code", json={"code": old_code}, headers=auth_headers(student))
    assert old_resp.status_code == 404

    new_resp = client.post("/classes/join-by-code", json={"code": new_code}, headers=auth_headers(student))
...
def test_regenerate_code_forbidden_for_non_owner_non_admin(client, db_session):
    ...
    resp = client.post(f"/classes/{cls.id}/regenerate-code", headers=auth_headers(other_teacher))
