...
def test_uploads_require_valid_signature(client):
    ...
    assert client.get(f"/uploads/{name}").status_code in (403, 422)

    exp, sig = make_signature(name)
    ok = client.get(f"/uploads/{name}", params={"exp": exp, "sig": sig})
    assert ok.status_code == 200 and ok.text == "secret-body"

    # подделанная подпись
    assert client.get(f"/uploads/{name}", params={"exp": exp, "sig": "deadbeef"}).status_code == 403
    # просроченная подпись
    exp2, sig2 = make_signature(name, ttl=-10)
    assert client.get(f"/uploads/{name}", params={"exp": exp2, "sig": sig2}).status_code == 403


def test_uploads_path_traversal_blocked(client):
    exp, sig = make_signature("../security.py")
    r = client.get("/uploads/../security.py", params={"exp": exp, "sig": sig})
...
def test_json_upload_urls_are_signed(client, db_session):
    ...
    r = client.get(f"/classes/{cls.id}", headers=auth_headers(teacher))
...
def test_non_owner_teacher_cannot_modify_class(client, db_session):
    ...
    assert client.put(f"/classes/{cls.id}", json={"name": "X"}, headers=auth_headers(other)).status_code == 403
    assert client.delete(f"/classes/{cls.id}", headers=auth_headers(other)).status_code == 403
    assert client.put(f"/classes/{cls.id}", json={"name": "X"}, headers=auth_headers(owner)).status_code == 200
...
def test_non_owner_teacher_cannot_modify_or_grade_assignment(client, db_session):
    ...
    assert client.put(f"/assignments/{a.id}", json={"title": "Z"}, headers=auth_headers(other)).status_code == 403
    assert client.delete(f"/assignments/{a.id}", headers=auth_headers(other)).status_code == 403

    sub = client.post(f"/assignments/{a.id}/submit", json={"text_content": "ans"}, headers=auth_headers(student))
    assert sub.status_code == 201
    sid = sub.json()["id"]
    assert client.post(f"/submissions/{sid}/grade", json={"score": 50}, headers=auth_headers(other)).status_code == 403
    assert client.post(f"/submissions/{sid}/grade", json={"score": 50}, headers=auth_headers(owner)).status_code == 201
...
def test_variant_delete_idor(client, db_session):
    ...
    assert client.delete(f"/assignments/{aA.id}/variants/{var.id}", headers=auth_headers(other)).status_code == 403
    # и не удалит, подставив свой assignment_id (вариант не принадлежит ему)
    assert client.delete(f"/assignments/{aB.id}/variants/{var.id}", headers=auth_headers(other)).status_code == 404
    # владелец удаляет корректно
    assert client.delete(f"/assignments/{aA.id}/variants/{var.id}", headers=auth_headers(owner)).status_code == 204
...
def test_rating_requires_membership_and_hides_email(client, db_session):
    ...
    assert client.get(f"/classes/{cls.id}/rating", headers=auth_headers(outsider)).status_code == 403
    r = client.get(f"/classes/{cls.id}/rating", headers=auth_headers(student))
    assert r.status_code == 200
    assert "@example.com" not in r.text

    # глобальный рейтинг тоже не раскрывает e-mail
    r = client.get("/rating", headers=auth_headers(student))
...
def test_members_requires_membership(client, db_session):
    ...
    outsider = make_user(db_session)
    cls, _ = _assignment(db_session, owner)
    assert client.get(f"/classes/{cls.id}/members", headers=auth_headers(outsider)).status_code == 403
...
def test_file_text_requires_signature(client, db_session):
    ...
    assert client.get("/upload/utils/file-text", params={"url": unsigned}, headers=auth_headers(user)).status_code == 403
