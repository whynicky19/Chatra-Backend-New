...
pptx), как и для сдачи студента. Регресс на найденный при аудите баг:

_resolve_reference_material: эталон учителя читался только как текст,
...
def test_deadline_checker_uses_assignment_reference_same_as_manual_path(db_session, monkeypatch):
    """Автопроверка по дедлайну и ручная кнопка "Проверить ИИ" должны
    резолвить один и тот же эталон задания."""
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
    ref_data = _docx_with_image("Эталон задания.", _png_bytes((5, 5, 5)))
    ...
    _, assignment = _make_assignment(
        db_session, teacher, reference_solution_url="http://localhost:8000/api/uploads/ref.docx"
    )

    student = make_user(db_session, role="student")
    sub = Submission(
        assignment_id=assignment.id, student_id=student.id,
        text_content="Ответ студента.", status="submitted",
    )
    db_session.add(sub)
    db_session.commit()
    ...
    assert "Эталон задания." in text_blocks
    assert any(p.get("type") == "image_url" for p in user_msg)
