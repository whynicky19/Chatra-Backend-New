    ...
    if body.lecture_context:
        payload["messages"].insert(0, {
            ...
            "content": (
                ...
                "именно его содержание. Если для лекции есть текст "
                "прикреплённого файла (идёт после строки [Файл \"...\"]) — "
                "объясняй строго по нему: заголовок и текстовое описание "
                "лекции учитель мог написать как угодно, и они могут не "
                "совпадать с реальным содержимым файла — доверяй файлу, а не "
                "им. Если у файла явно нет текста (например пометка "
                "\"содержимое НЕ прочитано\"), не выдумывай его содержание. "
                "Если содержимое файла ЯВНО не по теме заголовка/описания "
                "лекции (например это другой документ, код, инструкция — не "
                "относится к заявленной теме), прямо скажи об этом "
                "пользователю в начале ответа (например \"обратите внимание: "
                "прикреплённый файл не похож на материал по теме X, вот его "
                "содержание...\"), а не игнорируй расхождение и не притворяйся, "
                "что объясняешь тему по несуществующему материалу. "
