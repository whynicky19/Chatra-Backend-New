...
MAX_TITLE_LEN = 500
MAX_NAME_LEN = 256
# Название критерия оценивания: преподаватели нередко формулируют критерий
# развёрнуто — с перечислением требований, правилами оценивания и форматом
# ответа прямо в тексте критерия (это осмысленный сценарий: текст критерия
# целиком попадает в промпт ИИ-проверки). Старого лимита 256 не хватало,
# задание не сохранялось с 422. Колонка БД — Text, так что потолок разумный,
# а не бесконечный (защита от промпта-простыни на метры).
MAX_CRITERION_LEN = 5000
...
class CriterionIn(BaseModel):
    # MAX_CRITERION_LEN, не MAX_NAME_LEN: развёрнутая формулировка критерия
    # (требования + правила оценивания) легко переваливала за 256 символов,
    # и задание отказывалось создаваться.
    name: str = Field(max_length=MAX_CRITERION_LEN)
